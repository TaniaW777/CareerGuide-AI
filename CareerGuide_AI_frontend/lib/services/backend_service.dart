...
class BackendService {
  ...
  static Uri get _modelStatusUri => BackendConfig.modelStatusUri();
  ...
  static Future<Map<String, dynamic>> getRecommendations(Map<String, dynamic> profile) async {
    try {
      debugPrint("Frontend: Tentative d'envoi du profil au backend: ${jsonEncode(profile)}");
      ...
      debugPrint("Frontend: Réponse backend reçue avec code: ${response.statusCode}");
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("Frontend: ${items.length} recommandations normalisées.");
        ...
        debugPrint("Frontend: Erreur backend non-200: ${response.body}");
      }
    } catch (e) {
      debugPrint("Frontend: Erreur lors de la récupération des recommandations: $e");
    }

    debugPrint("Frontend: Passage au mode offline (Cache)");
